...
final marketRepositoryProvider = Provider((ref) => MarketRepository(ref.read(supabaseClientProvider)));

final bucketProvider = StateProvider<MarketBucket>((ref) => MarketBucket.all);
final marketQueryProvider = StateProvider<String>((ref) => '');
final marketSortProvider = StateProvider<MarketSort>((ref) => MarketSort.createdDesc);
final marketFiltersProvider = StateProvider<MarketFilters>((ref) => const MarketFilters());
...
final marketListingsProvider = FutureProvider<List<ListingModel>>((ref) {
  ...
  return ref
      ...
      .fetchListings(bucket: bucket, query: query, sort: sort, filters: filters);
});
...
final storeListingsProvider = FutureProvider.family<List<ListingModel>, String>((
  ...
  return ref.read(marketRepositoryProvider).fetchStoreListingsByUserId(store!.userId!);
});
...
final storeCardListingsProvider =
    FutureProvider.family<StoreCardListingData?, ({String storeSlug, int cardId})>((ref, key) async {
      ...
      final listingsFuture = repo.fetchCardListingsForSeller(cardId: key.cardId, card: card, store: store);
      final Future<({double? positivePct, int feedbackScore})> reputationFuture = store.userId == null
