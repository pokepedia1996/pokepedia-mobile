...
/// One owned card inside a [CollectionExpansionGroup].
...
/// The user's owned cards for one expansion, with the completion counts the
/// profile's progress bar shows.
class CollectionExpansionGroup {
  const CollectionExpansionGroup({
  ...
  String get slug => expansionCode.toLowerCase();

  /// Distinct cards owned — variants of the same card count once, like
  /// web's `uniqueCardIds` set.
  int get ownedCount => cards.map((c) => c.cardId).toSet().length;

  int get progressPercent =>
      totalCards > 0 ? ((ownedCount / totalCards) * 100).round() : 0;

  CollectionExpansionGroup withCards(List<CollectionCardEntry> next) {
    return CollectionExpansionGroup(
